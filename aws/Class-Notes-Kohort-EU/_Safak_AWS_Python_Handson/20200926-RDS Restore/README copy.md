...
- Ubuntu secerek bir EC2 Instance kuruyor ve uzerinde MariaDB Client kuruyoruz. MariaDB MySQL ile uyumlu. Security Group inbound MySQL/Aurora ve SSH ayarla. SSH birseyler indirip yuklemek icin lazim.
- Ubuntu kurulu EC2 Instance dan RDS e baglanacagiz.
- Database e offices ve employees tablolarini yukledikten sonra manuel snapshot alacagiz, sonra tablolardan bazi seyleri silecegit ve restore ile database i geri getirmeye calisiyoruz.
- Manuel alinmis bir snapshot tan geri donmek istedigimizde mevcut RDS Instance i kullanmiyor. Snapshot menusunde sag ust Actions sekmesinden restore snapshot a basinca snapshot in konulacagi yeni RDS instance in ayarlari yapilir. Bu demek oluyor ki yeni bir RDS endpoint imiz olacak.
- mysql de table silme komutu: drop table tablonun_adi
- mysql de table gosterme komutu: show tables
- mysql de table kullanma komutu: use table_adi
- Akrebin 3'e gelmesi Snapshot
- Yelkovanin 30'a gelmesi Transaction log : Point in Time Recovery. Transaction log lar ile saniye seviyesinde geri döndürüyoruz
- Point in Time Recovery de ayni manuel snapshot gibi mevcut RDS Instance haricinde yeni bir RDS instance olusturur. Mevcut RDS in menüsünde sag ustteki Actions kismindan restore to point in time sekmesine basinca yeni RDS instance in aralari yapilir.
- Public Access in No secilecegi zamanlar da olacak. Ornegin bir web sayfasinin database i Private VPC de tutulacak.
- mysqldump -h [restored-from-point-in-time-RDS endpoint] -u admin -p clarusway > backup.sql
Yukaridaki komut ile saglam olan RDS instance daki clarusway database bir backup.sql dosyasina dump edilir (kopyalanir). Bu dosyayi restore edilecek RDS instance a tasiyacagiz ki, silinen veriler geri gelebilsin
- mysql -h [RDS-mysql endpoint] -u admin -p migration < backup.sql
Yukaridaki komut ile saglam bigilerin oldugu dump edilmis olan backup.sql dosyasi arizalanmis olan ana RDS instance uzerinde yaratilan migration database ine aktarilir. Arizalanmis RDS instance uzerinde clarusway database e bakarsak employees table yok, yeni tasidigimiz migration database e bakarsak employees table var. restored-from-point-in-time sayesinde veri kurtarilmis oldu.
...
```
- clarusway db nin icinde herhangi bir bilgi yok. Asagida birseyler ekleyecegiz
