...
resource "aws_instance" "tf-nexus-server" {
  ...
  key_name      = "tyler-team" # Jenkinsi kullananlar Rancher icin olusturdugumuz pem i kullanacagiz
...
resource "null_resource" "forpasswd" {
  ...
  # sleep 3 m yerine girilen EC2 instance bekleme komutu
  # provisioner "local-exec" {
  #   command = "aws ec2 wait instance-status-ok --instance-ids ${aws_instance.tf-nexus-server.id}"
  # }

  # Asagidaki Terraform komutunun "ssh -i" ile baslayan kisnini Jenkins Server a bagliyken Terminalden girersek Nexus Server icin gerekli olan initial password u alabiliriz. 
  # Ya da Jenkins Serverda ilgili container a exec ile baglanip "docker cp" ile baslayan kisimndan itibaren girersek Nexus Server icin gerekli olan initial password u alabiliriz. 
